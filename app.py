    ...
    if register_button:
        if not selected_type or not passage_content.strip():
            ...
            if not is_duplicate:
                ...
                # 5-3. 입력창 초기화 (연속 등록 모드 체크 여부에 따라)
                if not st.session_state.continue_registration:
                    # 분류까지 초기화하려면 전체 페이지를 새로고침 (간단한 방법)
                    st.rerun() 
                else:
                    # 지문 내용만 초기화하고 번호만 다음 번호로 자동 세팅 (문항 번호 +1)
                    st.session_state["st_text_area"] = ""
                    if selected_type == "모의고사 및 수능":
                        # 모의고사는 번호 옵션이 정해져 있어 다음 번호 자동 지정이 복잡하므로, 일단 초기화만 합니다.
                        pass 
                    elif selected_type == "부교재" and isinstance(number, (int, float)):
                        try:
                            # 문항 번호 1 증가
                            st.session_state.sub_number = int(number) + 1
                        except:
                            # 에러 발생 시 초기화
                            st.session_state.sub_number = 1
                    
                    # 지문 내용만 비워주기
                    st.toast("지문 내용만 초기화되었습니다. 나머지 분류는 유지됩니다.")
                    # Streamlit의 Session State는 rerun() 없이 자동으로 업데이트 됨.
...
# --- 지문 내용 변경 시 분류 초기화 방지 로직 (만능형의 필수 전제) ---
# 이탈 방지 기능 (저장하지 않고 분류 변경/페이지 이탈 시)
# Streamlit의 특성상 간단한 코드로 구현이 어렵습니다. 
# 현재는 st.rerun()을 통해 의도적으로 페이지 전체를 다시 로드하고 있습니다.
# 예를 들어, 대분류가 바뀔 때 지문 내용이 비어있지 않으면, 지문 내용도 비우도록 합니다.
if passage_content and not st.session_state.get("last_selected_type") == selected_type:
    st.session_state["st_text_area"] = ""
    st.toast("지문의 종류를 변경하여 지문 내용이 초기화되었습니다. (데이터 보호)")
